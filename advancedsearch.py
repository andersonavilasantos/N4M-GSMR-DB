...
class AdvancedSearchPage:
    ...
    def render(self):
        ...
        for col in selected_columns:
            # Use self.filtered_data here
            selected_values = st.multiselect(f"Filter by {col}", self.filtered_data[col].unique())
            if selected_values:
                # Apply filter immediately to self.filtered_data
                self.filtered_data = self.filtered_data[self.filtered_data[col].isin(selected_values)]
                
                # Optional: add the filter to the self.filters dictionary
        ...
        graph_plots = GraphPlots(self.filtered_data)
